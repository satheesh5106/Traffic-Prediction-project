...
// Removed polars-js dependency

// Define interfaces for prediction results
...
export class TrafficPredictionService {
  ...
  private async initializeService() {
    try {
      ...
      if (fs.existsSync(`${this.modelPath}/model.json`)) {
        logger.info('Loading TensorFlow.js GNN model...');
        this.model = await tf.loadLayersModel(`file://${this.modelPath}/model.json`);
        logger.info('TensorFlow.js GNN model loaded successfully');
      } else {
        logger.warn('TensorFlow.js model not found, will use Python subprocess for inference');
      }

      // Load fallback data
      this.loadFallbackData();
    } catch (error) {
      logger.error('Failed to initialize traffic prediction service:', error);
    }
  }
  ...
  private loadFallbackData() {
    try {
      ...
      if (fs.existsSync(fallbackPath)) {
        ...
        logger.info('Fallback prediction data loaded successfully');
      } else {
        ...
        logger.info('Created default fallback prediction data');
      }
    } catch (error) {
  ...
  async predictTraffic(
    ...
    try {
      ...
      logger.info(`Traffic prediction completed in ${responseTime} seconds`);
      ...
      const result: TrafficPrediction = {
        flowData: (predictionObj as any).flowData || {
          congestionLevel: 'medium',
          speedFactor: 0.7,
          density: 'moderate'
        },
      ...
      logger.error('Error in traffic prediction:', error);
      throw new ApiError(500, 'Failed to generate traffic prediction');
    }
  }
   ...
   */
  private prepareInputData(latitude: number, longitude: number, liveTrafficData: any, weatherData: any, timeframe: number) {
    ...
    try {
      ...
      logger.error('Error preparing input data:', error);
   ...
   */
  private processCategoricalFeatures(data: any) {
    ...
    try {
      ...
      logger.error('Error processing categorical features:', error);
   ...
   */
  private async predictWithTensorflow(inputData: any) {
    try {
      ...
      logger.error('Error in TensorFlow.js prediction:', error);
      throw error;
    }
  }
   ...
   */
  private async predictWithPythonSubprocess(inputData: any) {
    return new Promise((resolve, reject) => {
      try {
        // Check if Python script exists
        if (!fs.existsSync(this.pythonScriptPath)) {
          logger.error('Python script not found:', this.pythonScriptPath);
        ...
        pythonProcess.stderr.on('data', (data) => {
          ...
          logger.warn('Python subprocess warning:', data.toString());
        });
        ...
        logger.error('Error in Python subprocess prediction:', error);
        reject(error);
      }
    });
  }
   ...
   */
  private processPredictionResults(predictionArray: number[] | number[][] | any): TrafficPrediction {
    ...
    try {
      ...
      if (Array.isArray(predictionArray)) {
        if (Array.isArray(predictionArray[0])) {
          ...
          flatArray = predictionArray as number[];
        }
      } else {
      ...
      if (congestionLevel < 0.5) congestionText = 'low';
      else if (congestionLevel < 1.5) congestionText = 'medium';
      else if (congestionLevel < 2.5) congestionText = 'high';
      else congestionText = 'severe';
      
      // Map density based on congestion and speed
      let density = 'moderate';
      if (congestionLevel < 1 && speedFactor > 0.8) density = 'light';
      else if (congestionLevel > 2 && speedFactor < 0.5) density = 'heavy';
   ...
   */
  async getFallbackPrediction(latitude: number, longitude: number) {
   ...
   */
  async getHistoricalTraffic(latitude: number, longitude: number, startDate: Date, endDate: Date) {
    try {
      ...
      throw new ApiError(500, 'Failed to retrieve historical traffic data');
    }
  }
   ...
   */
  private getRandomCongestionLevel(hour: number) {
   ...
   */
  private getRandomSpeedFactor(hour: number) {
    let min, max;
   ...
   */
  private getRandomDensity(hour: number) {
