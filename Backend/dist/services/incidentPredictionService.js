...
exports.incidentPredictionService = void 0;
...
class IncidentPredictionService {
     ...
     */
    predictIncident(factors) {
        try {
            logger_1.logger.info('Starting incident prediction', { factors });
            ...
            logger_1.logger.info('Incident prediction completed', { result });
            return result;
        }
        catch (error) {
            logger_1.logger.error('Error in incident prediction', { error, factors });
     ...
     */
    getVehicleTypeRisk(vehicleType) {
        const riskMap = {
            1: 0.9, // Pedal cycle
     ...
     */
    determineSeverity(riskScore, factors) {
     ...
     */
    calculateConfidence(factors, riskScore) {
        let confidence = 0.8; // Base confidence
        // Reduce confidence for edge cases
        if (factors.age < 16 || factors.age > 90)
            confidence -= 0.1;
        if (factors.speed > 150)
            confidence -= 0.1;
        if (factors.engineCapacity > 5000)
            confidence -= 0.05;
        // Increase confidence for common scenarios
        if (factors.vehicleType === 9)
            confidence += 0.05; // Cars are well-studied
        if (factors.weather === 1 && factors.light === 1)
            confidence += 0.05; // Clear conditions
     ...
     */
    determineRiskLevel(severity, confidence) {
        if (severity === 3 && confidence > 0.7)
            return 'CRITICAL';
        if (severity === 3 || (severity === 2 && confidence > 0.8))
            return 'HIGH';
        if (severity === 2 || (severity === 1 && confidence < 0.6))
            return 'MEDIUM';
        return 'LOW';
    }
}
// Export singleton instance
exports.incidentPredictionService = new IncidentPredictionService();
//# sourceMappingURL=incidentPredictionService.js.map