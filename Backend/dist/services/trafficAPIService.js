...
exports.TrafficAPIService = void 0;
...
class TrafficAPIService {
    constructor() {
        ...
        if (!this.mapMyIndiaApiKey) {
            logger_1.logger.warn('MapMyIndia API key not set. Traffic data will be simulated.');
        }
    }
    /**
     * Get live traffic data for a location
     */
    async getLiveTraffic(latitude, longitude, radius = 2000) {
        ...
        try {
            ...
            if (this.mapMyIndiaApiKey) {
                ...
                const response = await axios_1.default.get(`https://apis.mappls.com/advancedmaps/v1/${this.mapMyIndiaApiKey}/traffic_flow?lat=${latitude}&lng=${longitude}&radius=${radius}`, { timeout: 5000 });
            ...
            logger_1.logger.error('Error fetching live traffic data:', error);
     ...
     */
    async getTrafficIncidents(latitude, longitude, radius = 5000) {
        ...
        try {
            ...
            if (this.mapMyIndiaApiKey) {
                ...
                const response = await axios_1.default.get(`https://apis.mappls.com/advancedmaps/v1/${this.mapMyIndiaApiKey}/traffic_incidents?lat=${latitude}&lng=${longitude}&radius=${radius}`, { timeout: 5000 });
            ...
            logger_1.logger.error('Error fetching traffic incidents:', error);
            // Return simulated data as fallback
     ...
     */
    processMapMyIndiaResponse(responseData) {
        try {
            ...
            logger_1.logger.error('Error processing MapMyIndia response:', error);
     ...
     */
    processMapMyIndiaIncidentsResponse(responseData) {
        try {
            // This is a placeholder implementation as the actual API response format may vary
            // Adjust according to the actual MapMyIndia API response structure
            ...
            return responseData.data.map((incident) => ({
            ...
            logger_1.logger.error('Error processing MapMyIndia incidents response:', error);
            return [];
        }
    }
    /**
     * Generate simulated traffic data
     */
    generateSimulatedTrafficData(latitude, longitude) {
     ...
     */
    generateSimulatedRoadSegments(latitude, longitude, baseCongestion) {
        const segments = [];
     ...
     */
    generateSimulatedIncidents(latitude, longitude) {
        const incidents = [];
...
exports.TrafficAPIService = TrafficAPIService;
//# sourceMappingURL=trafficAPIService.js.map