...
export class KDTree<T> {
   ...
   */
  constructor(
    points: Array<PointWithData<T>>,
    distanceFn: (a: Point, b: Point) => number = euclideanDistance
  ) {
   ...
   */
  public nearest(target: PointWithData<any>, k: number = 1): Array<PointWithData<T>> {
    if (!this.root || k <= 0) {
      return [];
    }

    const targetPoint = target.slice(0, -1) as Point;
    const nearestPoints = new BoundedPriorityQueue<PointWithData<T>>(k);
   ...
   */
  private buildTree(points: Array<PointWithData<T>>, depth: number): KDNode<T> {
    if (points.length === 0) {
    ...
    // Create node
    const node: KDNode<T> = {
      point: median.slice(0, -1) as Point,
      data: median[median.length - 1] as T,
      left: null,
      right: null,
      axis
    };
  ...
  private nearestSearch(
    ...
    target: Point,
    depth: number,
    nearestPoints: BoundedPriorityQueue<PointWithData<T>>
  ): void {
    if (!node) {
      return;
    }
    ...
    const pointWithData: PointWithData<T> = [...node.point, node.data];
