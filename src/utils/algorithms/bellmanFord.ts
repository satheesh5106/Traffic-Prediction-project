 ...
 */
export function bellmanFord(edges: Edge[], start: string, end: string): BellmanFordResult {
  ...
  while (current) {
    ...
    if (path.indexOf(current as string) !== -1) {
 ...
 */
export function bellmanFordAll(edges: Edge[], start: string): Record<string, BellmanFordResult> {
  // Get all unique nodes from edges
  const nodes = new Set<string>();
  edges.forEach(edge => {
    nodes.add(edge.from);
  ...
  const result: Record<string, BellmanFordResult> = {};
