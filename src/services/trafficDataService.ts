...
import { GeoPoint, isWithinDistance } from '../utils/geo';
...
// Traffic data for a specific location
...
export interface TrafficIncident {
  ...
  endTime?: number;
  description: string;
  affectedArea?: GeoPoint[]; // Polygon defining affected area
  impactRadius?: number; // Impact radius in meters
}
...
interface TrafficDataServiceConfig {
...
export class TrafficDataService {
  ...
  private trafficIncidents: Map<string, TrafficIncident> = new Map();
  ...
  private subscribers: Set<(data: TrafficData[]) => void> = new Set();
  
  /**
   * Create a new traffic data service
   * @param config Service configuration
   */
  constructor(config: TrafficDataServiceConfig = {
    ...
    logger.info('TrafficDataService initialized', config);
  }
  ...
  public startUpdates(): void {
    ...
    logger.info('Starting traffic data updates');
  ...
  public stopUpdates(): void {
    if (this.updateTimer) {
      ...
      logger.info('Stopped traffic data updates');
    }
  }
  ...
  private async updateTrafficData(): Promise<void> {
    try {
      logger.debug('Updating traffic data');
      ...
      logger.debug(`Traffic data updated: ${this.trafficData.size} data points, ${this.trafficPredictions.size} predictions`);
    } catch (error) {
   ...
   */
  private generateTrafficPredictions(): void {
   ...
   */
  public getTrafficDataAtLocation(location: GeoPoint, radius: number = 500): TrafficData | null {
    // Check cache first
    const cacheKey = `traffic:${location.lat.toFixed(5)}:${location.lng.toFixed(5)}:${radius}`;
    const cached = this.cache.get<TrafficData>(cacheKey);
    ...
    let closestData: TrafficData | null = null;
    let minDistance = Infinity;
    
    for (const data of this.trafficData.values()) {
      if (isWithinDistance(location, data.location, radius)) {
        const distance = Math.sqrt(
          Math.pow(location.lat - data.location.lat, 2) +
   ...
   */
  public getTrafficPredictionAtLocation(location: GeoPoint, radius: number = 500): TrafficPrediction | null {
    // Check cache first
    const cacheKey = `prediction:${location.lat.toFixed(5)}:${location.lng.toFixed(5)}:${radius}`;
    const cached = this.cache.get<TrafficPrediction>(cacheKey);
    ...
    let closestPrediction: TrafficPrediction | null = null;
    let minDistance = Infinity;
    
    for (const prediction of this.trafficPredictions.values()) {
      if (isWithinDistance(location, prediction.location, radius)) {
        const distance = Math.sqrt(
          Math.pow(location.lat - prediction.location.lat, 2) +
   ...
   */
  public getAllTrafficIncidents(): TrafficIncident[] {
   ...
   */
  public addTrafficIncident(incident: TrafficIncident): void {
    this.trafficIncidents.set(incident.id, incident);
    logger.info(`Added traffic incident: ${incident.id}`, incident);
   ...
   */
  public removeTrafficIncident(id: string): boolean {
    const removed = this.trafficIncidents.delete(id);
    
    if (removed) {
      logger.info(`Removed traffic incident: ${id}`);
   ...
   */
  public subscribe(callback: (data: TrafficData[]) => void): () => void {
  ...
  private notifySubscribers(): void {
    ...
    for (const subscriber of this.subscribers) {
      try {
        ...
        logger.error('Error in traffic data subscriber', error);
      }
    }
  }
   ...
   */
  private getLocationKey(location: GeoPoint): string {
   ...
   */
  public getHistoricalTrafficData(location: GeoPoint, hours: number = 24): any[] {
   ...
   */
  public getActivePredictionsCount(): number {
   ...
   */
  public getAccuracyMetrics(): { accuracy: number } {
   ...
   */
  public getResponseTimeMetrics(): { averageResponseTime: number } {
...
export const trafficDataService = new TrafficDataService();